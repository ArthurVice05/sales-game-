/**
 * Baralho de Sorte & Revés — dados + resolução puros.
 *
 * Extraído de SorteRevesModal.jsx SEM alterar regra ou valor: o componente
 * passou a importar daqui. Isso torna as 34 cartas testáveis sem DOM.
 */
...
export const SORTE_REVES_CARDS = Object.freeze([
    // ----- SORTE -----
...
/**
 * Resolve o efeito de uma carta para um jogador.
 * Mesma lógica que o modal já usava para montar texto + payload.
 */
export function resolveCardEffect(card, player = {}) {
  ...
  if (typeof card._compute === 'function') {
    ...
    return { text: _overrideText || card.text, payload: { ...base, ...effect } }
  }
  const fixed = {}
